<template>
	<v-row>
		...
		<template v-else>
			<template
				v-if="searchResultDefault && searchResultDefault.length > 0"
			>
				<v-col
					v-for="n in searchResultDefault"
					:key="n.id_buku"
					lg="4"
					md="6"
					sm="12"
			...
			<template
				v-else-if="
					!searchResultDefault || searchResultDefault.length < 1
				"
			>
...
export default {
	...
	methods: {
		callFunction: function() {
			this.$store.dispatch("BookSearch/getSearchByDefault", this.payload);
			var v = this;
			setTimeout(function() {
	...
	computed: {
		...mapGetters("BookSearch", ["searchResultDefault"])
	},
	mounted() {
		this.callFunction();
	},
