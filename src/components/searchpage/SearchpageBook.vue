<template>
	<v-row>
		...
		<template v-else>
			<template
				v-if="searchResultByBook && searchResultByBook.length > 0"
			>
				<v-col
					v-for="n in searchResultByBook"
					:key="n.id_buku"
					lg="4"
					md="6"
					sm="12"
			...
			<template
				v-else-if="!searchResultByBook || searchResultByBook.length < 1"
			>
...
export default {
	...
	methods: {
		callFunction: function() {
			this.$store.dispatch("BookSearch/getSearchByBook", this.payload);
			var v = this;
			setTimeout(function() {
	...
	computed: {
		...mapGetters("BookSearch", ["searchResultByBook"])
	},
	destroyed() {
