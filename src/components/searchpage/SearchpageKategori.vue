<template>
	<v-row>
		...
		<template v-else>
			<template
				v-if="
					searchResultByKategori && searchResultByKategori.length > 0
				"
			>
				<v-col
					v-for="n in searchResultByKategori"
					:key="n.id_buku"
					lg="4"
					md="6"
					sm="12"
			...
			<template
				v-else-if="
					!searchResultByKategori || searchResultByKategori.length < 1
				"
			>
...
export default {
	...
	methods: {
		callFunction: function() {
			this.$store.dispatch(
				"BookSearch/getSearchByKategori",
				this.payload
			);
			var v = this;
			setTimeout(function() {
	...
	computed: {
		...mapGetters("BookSearch", ["searchResultByKategori"])
	},
	destroyed() {
